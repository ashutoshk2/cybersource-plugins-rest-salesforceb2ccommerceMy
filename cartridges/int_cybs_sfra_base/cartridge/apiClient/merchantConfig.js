...
MerchantConfig.prototype.defaultPropValues = function defaultPropValues() {
  var warningMessage = ""; //fallback for missing values
  ...
  if (warningMessage.length > 0) logger.warn(warningMessage);
  this.runEnvironmentCheck(logger); // verifying mandatory properties
  ...
  MerchantConfig.prototype.getAllProperties = function getAllProperties(merchantConfig) {
    ...
    for (var prop in merchantConfig) {
      if (hiddenProperties.indexOf(prop) === -1) {
        merchantMap[prop] = merchantConfig[prop];
      }
    }

    return newMerchantMap;
  };
};
